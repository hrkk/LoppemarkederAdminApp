...
<div class="fieldcontain ${hasErrors(bean: coreMarkedItemInstance, field: 'address', 'error')} required">
	...
	<g:select id="address" name="address.id" from="${dk.roninit.loppadmin.Address.listOrderByAddressLine1()}" optionKey="id" required="" value="${coreMarkedItemInstance?.address?.id}" class="many-to-one"/>
</div>
