		...
		<div id="show-${domainClass.propertyName}" class="content scaffold-show" role="main">
			...
			<%  excludedProps = Event.allEvents.toList() << 'id' << 'version'
                ...
                props.eachWithIndex { it, idx ->
                    if (it.name == 'createdBy') {
                        createdBy = it
                    }

                    if (it.name == 'dateCreated') {
                        dateCreated = it
                    }

                    if (it.name == 'modifiedBy') {
                        modifiedBy = it
                    }

                    if (it.name == 'lastUpdated') {
                        lastUpdated = it
                    }
                }

                props.remove(createdBy)
                props.remove(dateCreated)
				...
				<li class="fieldcontain">
                    ...
                    <%  } else if (p.type == BigDecimal) { %>
                        <span class="property-value" aria-labelledby="${p.name}-label"><g:formatNumber number="\${${propertyName}?.${p.name}}" type="number" maxFractionDigits="9" /></span>
